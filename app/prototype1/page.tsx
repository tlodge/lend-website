...
export default function Prototype1() {
  ...
  const formatEvent = (event: IframeEvent): string => {
    switch (event.type) {
      case 'view-change':
        return `View changed to: ${event.view}${event.metadata ? ` (${JSON.stringify(event.metadata)})` : ''}`
      case 'story-select':
        return `Story selected: ${event.storyTitle} (ID: ${event.storyId})`
      case 'story-reflection':
        return `Story feedback: ${event.response} for story ${event.storyId}`
      case 'menu-action':
        return `Menu action: ${event.action}`
      default:
